...
app.get('/api/get-calories', (req, res) => {
    ...
    if (user) {
        ...
        const schedule = generateSchedule(calories);
        res.json({ calories, schedule });
    } else {
        res.status(404).json({ error: 'User not found' });
    }
});

function generateSchedule(calories) {
    if (calories > 2500) {
        return [
            { day: 'Monday', activity: 'Long-distance run (5-7 km)' },
            { day: 'Wednesday', activity: 'HIIT workout (30 mins)' },
            { day: 'Friday', activity: 'Cycling session (10 km)' },
        ];
    } else if (calories > 2000) {
        return [
            { day: 'Monday', activity: 'Moderate run (3-5 km)' },
            { day: 'Wednesday', activity: 'Bodyweight exercises (40 mins)' },
            { day: 'Friday', activity: 'Swimming (30 mins)' },
        ];
    } else {
        return [
            { day: 'Monday', activity: 'Light jog (2-3 km)' },
            { day: 'Wednesday', activity: 'Walking (45 mins)' },
            { day: 'Friday', activity: 'Yoga (30 mins)' },
        ];
    }
}
